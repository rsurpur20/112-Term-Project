...
def timerFired(app):
    # print(app.movingEyesFeature, app.clickscrollingFeature)
    ...
    # mask1=cv2.inRange(lefthsv,app.lowerPupil,app.higherPupil)
    # resize1= cv2.resize(mask1,(400,150),interpolation =cv2.INTER_AREA)

    # righthsv=cv2.cvtColor(rightFrame, cv2.COLOR_BGR2HSV)
    # mask2=cv2.inRange(righthsv,app.lowerPupil,app.higherPupil)
    ...
    # cv2.moveWindow("left", 550,30)
    # cv2.moveWindow("right", 900,30)
...
def checkdisengage(app):
    # print(app.startDisengage)
    # print("time",time.time())
    if not app.currentlyCalibrating:
        # if app.movingEyesFeature or app.clickscrollingFeature:
            # print(app.eyesClosed, app.startDisengage)
        if app.leftPupil.area<=0 and app.rightPupil.area<=0: #both eyes are closed
            # app.eyesClosed=True
        # else: app.eyesClosed=False
            if not app.eyesClosed:
                app.startDisengage=time.time()
                app.eyesClosed=not app.eyesClosed

        # if app.notalreadyClosed and time.time()-app.startDisengage<2:
        #     print("blinked")
                    # app.startDisengage=0
                # else:
        # print(int(time.time()-app.startDisengage))
        # if app.eyesClosed:
        #     print("trying to disengage")

        if app.eyesClosed and 7<time.time()-app.startDisengage<10:
...
def redrawAll(app,canvas):
    ...
    canvas.create_text(app.width//2,start+135,text=f"Mouse Moving Mode: %s" %("On" if app.movingEyesFeature else "Off"))
